...
```make
여러 사람이 함께 작업을 할 경우
서로 작업한 코드가 서로에게 영향을 줄 수 있음
이것을 방지하기 위해 PR은 일일히 하나씩 체크하는 것이 좋다. (사실 상당히 빡센 작업임)

추후 class + method 케이스를 보면서 왜 문제가 되는지 살펴보도록 합니다.
...
불변해야 하는데 변해버렸기 때문에 이것으로 인한 파장은 모두가 겪게됨
```
