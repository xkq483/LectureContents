class RandomSalary {
    final int MAX = 10; //10명이기 때문에 고정 변수 10을 한다.
    final float PERCENT = 0.01f; //퍼센트계산을 위해 0.01f
    int[] yearSalary;  //연봉을 저장할 그릇

    public RandomSalary () {
        yearSalary = new int[MAX]; //10개의 그릇

        for (int i = 0; i < MAX; i++) { //안에 2400~3500을 담는다.
            yearSalary[i] = (int)(Math.random() * 1101 + 2400);
        }
    }
    public void calcRandomSalary (int year) {//() 안에 대입값을 넣는다고 한다.
        float randIncRate;
