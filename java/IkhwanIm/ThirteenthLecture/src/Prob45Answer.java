...
class DiceDeathGame {
    ...
    public void gameStart () {//이렇게 메소드들만으로 프로그램이 실행될 수 있구나
        do {               //서로가 상호작용이 되는구나,   하지만 순서가 맞아야된다
            bettingMoney();//배팅하는곳
            runDice();   //주사위 돌리는곳
            checkMagicDiceSkill(); //두번째주사위
            whoGetBetMoney(); //배팅금 차감
            canWePlay();    //배팅금 0원이하일 경우 거짓
            checkEachMoney(); //남은돈 출력
        } while (isAlive);//false가되면 stop
    }
    ...
    private void checkSkill (int[] curDice, int[] targetDice) {
        switch (curDice[SECOND_IDX]) {//curdice, targetdice로 설정하니까
            case 1:                  //두개의 스위치문을 하나로 줄일수가있군
    ...
    private void checkMagicDiceSkill () {//대입값만 서로 바꿔주면 되는군
    ...
    private void runSecondDice (int[] dice) {
        ...
        }  //겟랜덤밸류를 통해서 귀찮은 math.random을 생략할수있군
    }
...
public class Prob45Answer {
    public static void main(String[] args) {
        ...
        ddg.gameStart();// 게임스타트를 불러오는것 만으로 실행이 가능하다니
    }                   // 혁신이군
}