...
public class AllocHeapArray {
    public static void main(String[] args) {
        ...
        System.out.print("학급에 학생이 몇 명 입습니까? ");

        int studentNum = scan.nextInt();

        // 동적할당 되는 데이터를 관리하는 메모리 - Heap
        // new로 만든 데이터는 전부 Heap에서 관리하게 된다.
        // 기존에 우리가 stack에 배열을 만들 경우엔 항상 제약 상항이 발생했었다.
        // 어떤 제약 사항 ? 개수가 고정됨(할당된 값으로)

        // Heap 방식의 할당에서는 공산을 필요한 만큼 할당할 수 있어서 유연한 접근이 가능하다.
        // 근데 여기에도 제약사항이 있다? stack보다 느리다.

        // 새로운 개념 new를 통해 공간을 만드는 방법
        // 1. new를 적는다.
        // 2. 데이터 타입을 적는다.
        // 3. 만약 데이터타입이 배열이라면 대괄호를 열고 몇 갸 만들지 적는다.
        //    만약 데이터타입이 클래스라면 소괄호를 열고 닫고 필요하다면 인자를 설정한다.

        // Heap 공간에 int형 배열을 studentNum 개수민큼 만들겠습니다.
        // 그리고 studentArr는 Heap에 생성된 공긴은 제어히게 된다.
