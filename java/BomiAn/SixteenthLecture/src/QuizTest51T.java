...
class Market {
    ...
    private void doPayment () {
        ...
        int price;
        System.out.println("상품명 \t 구매수량 \t 합산금액");
        for (int i = 0; i < userBuyListStock.size(); i++) {
            String value = userBuyList.get(i);
            for (int j = 0; j < marketSellList.length; j++) {
                if (value == marketSellList[j]) {
                    price = marketSellListPrice[j] * userBuyListStock.get(i);
                    System.out.println(value + "       " + userBuyListStock.get(i) + "     " + price);
                    totalPrice += price;
                }
            }
        }
        System.out.println("================ 합계 : " + totalPrice);
        System.out.println("================ 잔액 : " + myMoney + " - " + totalPrice + " = " + (myMoney - totalPrice));
