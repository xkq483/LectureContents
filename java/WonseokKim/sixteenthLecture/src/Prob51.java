...
class Market {
    ...
    private void payBill () {
        System.out.printf("지갑의 있는 현금 :%d, 구매한 물품의 총 금액 :%d\n", myMoney, charge);
        int totalAmount = myMoney - charge;
        if(totalAmount < 0){
            System.out.println("잔액이 부족하여 물품을 구매할수 없습니다. 잔액을 채워주세요");
        } else if (totalAmount >= 0){
            System.out.println("구매가 완료 되었습니다. 감사합니다^^ ");
        }
    }

    private void payCheckBill() {
        Boolean isEnd = true;

        do {

            System.out.print("그럼 결재 진행 하시겠습니까? Y/N");
    ...
    private void doPayment () {
        ...
        for(int i = 0; i < userBuyList.size(); i++){
            ...
            if(userBuyList.get(i).equals("선풍기")){
                charge += userBuyListStock.get(i) * marketSellListPrice[0];
            }else if(userBuyList.get(i).equals("키보드")){
                charge += userBuyListStock.get(i) * marketSellListPrice[1];
            }else if(userBuyList.get(i).equals("마우스")){
                charge += userBuyListStock.get(i) * marketSellListPrice[2];
            }else if(userBuyList.get(i).equals("모니터")){
                charge += userBuyListStock.get(i) * marketSellListPrice[3];
            }
        }
