...
- Scanner 써서 만들기 -
    ...
    사원이름 직접입력
    시작 연봉 2400 ~ 3500 랜덤
    연봉 인상률은 1% ~ 20% 사이의 랜덤값
    10 년후의 각 직원들의 연봉을 출력하도록 프로그래밍
    또한 연별로 입력한 사원들의 평균 연봉값을 계산
 */
...
class Employee1 {
    final float PERCENT = 0.01f; // 상수

    String name; // 사원이름
    int[] pay; // 연봉
    float[] incRate; // 연봉 인상율

    int curYear; // 현재연차


    // 생성자 정의 (값 초기화)
    public Employee1 (String name, int year) {
        this.name = name;
        curYear = 0; // 현재 연차를 0으로 초기화 후 calcYearPay() 내에서 +1, 출력메소드에서 +1

        pay = new int[year];
        incRate = new float[year];

        pay[0] = (int)(Math.random() * 1101 + 2400); // 랜덤 초봉값
    }
    ...
    // 연봉인상율을 적용시켜 연봉배열에 담는 메소드
    public void calcYearPay () {
        for (int i = curYear + 1; i < pay.length; i++) { // curYear + 1; : 0이 아닌 1년차 연봉부터 구하기 때문
            pay[i] = pay[i - 1] + (int)(pay[i - 1] * getRandomIncRate() * PERCENT);
            // 다음해 연봉 = 현재연봉 + (현재연봉 * 인상율%)
        }
    }
    ...
    // 랜덤 인상율 값 반환한 것을 calcYearPay()에서 사용할 메소드
...
public class review_ScannerClassRoom {
    ...
    public static float[] calcEmployeeMean(Employee1[] emp) {
        int[] pay;
    ...
    public static void main(String[] args) {
        final int YEAR = 10; // 10년까지의 연봉만 구하려 하기때문에 상수로

        System.out.print("몇 명의 학생 정보를 입력할까요 ? ");
        int studentNum = scan.nextInt();
        scan.nextLine();

        Employee1[] emp = new Employee1[studentNum];
        ...
        for (int i = 0; i < studentNum; i++) {
            emp[i] = new Employee1(inputName(), YEAR);
            /*
