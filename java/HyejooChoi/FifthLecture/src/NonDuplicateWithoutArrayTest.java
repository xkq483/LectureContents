public class NonDuplicateWithoutArrayTest {
    // 0 ~ 9 까지의 숫자가 중복되어 나오지 않게 만들기 (배열없이)
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            // 나온 random에 대한 중복 판정을 어떻게 할 것인가? bit 연산
            // 2^9      2^8     2^7     2^6     2^5     2^4     2^3     2^2     2^1     2^0

            // BIN = 2로 시작하면 2^1 에서 시작하기 때문에
            // 맨 마지막값이 무조건 2^1 x 2^9가 돼서 1024가 더해지므로 논리적 오류가 발생
            // 그러므로 BIN = 1로 만들어서 2^0 x 2^9로 512가 되게 처리해야 정상결과 나옴
            ...
            /*
                testBit는 int형라서 전체가 4바이트(32비트)
                ex) 4, 5, 4, 1
             */
            while ((testBit & (BIN << randNum)) != 0) {
                System.out.println("중복이 많이 발생 : " + randNum);
                randNum = (int)(Math.random() * 10);
            }
