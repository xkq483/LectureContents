class Teacher {
    ...
    // 마우스 커서를아래 갖다 놓고 Alt + Insert를 누른다
    // Gatter & Setter -> 전부 선택하고 OK 하면 자동으로 Setter와 Getter가 만들어진다.
    ...
    // 결론 : 값을 얻는 목적으로 사용하는것이 Getter
    int getAge() {
        return age;
    }

    // 회원가입시 집 주소 적으라고 나오는데 여기서 입력하는 값들을 처리하는게 Setter에 해당한다
    // 특정한 값을 설정하는 못적으로 Setter가 사용된다.
    // 결론 : 값을 설정하는 목적으로 사용하는것이 Setter
    void setAge(int age) {
...
public class ClassMethodTest {
    public static void main(String[] args) {
        ...
        teacher.setAge(40); // 40이 입력
        teacher.setName("GAORICK");
        teacher.setMajor("Physics");

        // 설정된 값을 Getter를 통해 얻는다
        System.out.printf("%s 는 %s를 전공하였고 %d 세다.\n", teacher.getName(), teacher.getMajor(), teacher.getAge());

        // teacher.name, teacher.major, teacher.age 로 출력해도 보입니다.
    }
}
