public class NonDuplicateWithoutArrayTest {
    ...
    public static void main(String[] args) {
        ...
        // 2. 찾는 숫자 10에서 8을 뺀 값인 2를 적는다. => 10 - 8 = 2
        // 3. 값이 0이 나올때까지 이 절차를 반복한다 => 2 - 2 = 0
        // 4. 0이 된 이후 뺏건 값들의 2^n에 해당하는 n 값들을 열거한다 => 2^3 = 8, 2^1 =2 여기서 n에 해당하는 값은 3 과 1
        ...
        for(int i = 0; i < 10; i ++) {
            ...
            // Ex) testBit는 int형이니까 전체가 4바이트(32비트)
            //     여기서 4, 5, 4, 1 을 출력 한다고 하면?
            //                                     1
            //                              1
            //                                     0
            //                                                          1
            ...
            // 맨 마지막값이 무조건 2^1 x 2^9이 되어서 1024가 더해지므로 논리적 오류가 발생함
            // 그러므로 BIN = 1 로 만들어서 2^0 x 2^9 으로 512가 되게 처리해아 정상적인 결과가 나옴

            // 쉬프트 연산의 결과 (비트를 왼쪽에서 이동시킴)
...
// 하나라도 0(거짓)이 있으면 해당자리수는 0(거짓)이 된다

// 비트 연산자 OR는 각 비트의 자리수중 하나라도 1(참)이 있으면 1(참)이 된다
// 양쪽 모두 0(거짓)이 있으면 해당자리수는 0(거짓)이 된다
