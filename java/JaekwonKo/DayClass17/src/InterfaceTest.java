...
// 객체란 단어만 보고도 이것이 어떻게 형성되었는지 등이 이미 뇌리에 스치고 있음
...
class AbstractTest {
    ...
    Remocon radio = new Remocon() {
        @Override
        public void turnOn() {
            System.out.println("나는 라디오야 ! 지금부터 주파수 매칭을 시작할게!");
        }

        @Override
        public void turnOff() {
            System.out.println("이제 안녕! 주파수 채널 매칭을 끊을게!");
        }
    };

    public void testMethod () {
        Remocon t v = new Remocon() {
            @Override
            public void turnOn() {
                System.out.println("나는 TV야! AM/FM 신호를 수신할게! 이제부터방송을 보자!");
            }
