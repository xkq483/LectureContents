class Teacher {
    ...
    // 마우스 커서를 아래(클래스 내부에) 갖다놓고 Alt + Insert를 누른다.
    // Getter & Setter -> 전부 선택하고 OK하면 Setter와 Getter 가 만들어진다.
    // 만약 키보드 설정이 안맞는 경우 Code -> Generate로 같은 작업을 할 수 있다.

    // 매서드를 만드는 방법
    // 1. 먼저 리턴(returen) 타입을 작성한다.
    ...
    // 소괄호 내부에는 인자가 배치되는데 텅 비어있는 것은 인자(입력)이 없다는 뜻이다.
    // 결론 : 값을 얻는 목적으로 사용하는 것이 Getter
    int getAge() {
        return age;
    }

    // 회원가입 시 집 주소 적으라고 나오는데 여기서 입력하는 값들을 처리하는게 Setter에 해당한다.
    // 특정한 값을 설정하는 목적으로 Setter가 사용된다.
    // 결론 : 값을 설정하는 목적으로 사용하는 것이 Setter
    void setAge(int age) {
        // this.age는 클래스 내부에 있는 age를 의미한다.
        // age는 입력으로 들어온 age에 해당한다.(void~ age에있는 것)
        this.age = age;
    }
    String getName() {
        return name;
...
public class e_ClassMethodTest {
    public static void main(String[] args) {
        ...
        System.out.printf("%s는 %s를 전공했고, %d세다.\n", t.getAge(), t.getMajor(), t.getAge());
