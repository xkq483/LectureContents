...
class ThreePoker {
    ...
    private Boolean checkTrippleNumber (int[][] arr, int identity) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 4; j++) {
                if (((arr[0][j] == arr[1][j]) && (arr[i][j]>-1)) || ((arr[0][j] == arr[2][j]) && (arr[i][j]>-1))){
                    numCnt++;
                }
                //이러면 검사하는 범위끼리의 같은 패턴을 카운트하게된다..

                if ((arr[1][j] == arr[2][j]) && (arr[i][j]>-1)) {
                    secondaryNumCnt++;
                }
