...
class ThreePoker {
    ...
    public void distributeCard (int[][] arr, int identity) {
        ...
        for (int i = 0; i < 4; i++) {
            do {
                ...
                randNum = (int) (Math.random() * 5); //일치하는 숫자가 3개가 나오는 경우가 너무 적어서 10을 7로 변경
    ...
    private Boolean checkTrippleWeapon (int[][] arr, int identity) {
        // 1. 문양도 일치해야하고
        // 2. 숫자가 연속되어야함
        if (!checkTripplePattern(arr, identity)) { //조건을 부정하는 !가 있으니, false가 와야 true이다. if()안이 true일때 리턴이 작동한다.
            return false;
        }
        ...
        return true; //트리플 패턴과 트리플 하모닉이 모두 true이면 이 메소드가 true를 반환하는것인가?
        //이 부분이 이해가 잘안됩니다. 트리플 패턴은 같은 문양 3개이고 하모닉은 다른 문양, 연속 숫자 3개인데
        //둘다 만족해서 true가 나올 수 있나요?
    }
    ...
    private Boolean checkTrippleNumber (int[][] arr, int identity) {
        ...
        System.out.println("다른 문양, 3번 일치하는 일치하는 숫자 개수 = " + numCnt);
        //System.out.println(pattern[savedRow[1]]);
        //System.out.println("다른 문양, 두 번째 숫자와 일치하는 개수 = " + secondaryNumCnt);

        if (numCnt >= 2 || secondaryNumCnt >= 2) {
            System.out.println("다른 문양, 동일한 숫자 3개 이상 검출되었습니다!");
            return true;
        }
        return false;
    }

    private Boolean checkTrippleHarmonic (int[][] arr, int identity) { //짰는데 올바르게 검출을 못하네요 ㅠㅠ
        int checkNum;
                ...
                if(arr[0][j] != -1) {
                    ...
                    for(int k=1; k<3; k++) {
                        for(int l=0; l>4; l++) {
                            ...
                            if(checkNum + 1 == arr[1][l]) { //내려가면서 순차적으로 커지는 숫자가 있는지 검사.
                                for(int m=0; m<4; m++) {
                                    if((checkNum + 2 == arr[2][m]) || ((checkNum -1  == arr[2][m]) && (checkNum -1 != -1))) {
                                        System.out.println("다른 문양, 연속된 숫자가 발생했습니다!");
                                        return true; // 1 2 3 || 2 3 1
                                    }
                                }
                            }
                            if(checkNum + 1 == arr[2][l]) {
                                for(int m=0; m<4; m++) {
                                    if(checkNum + 2 == arr[1][m]) {
                                        System.out.println("다른 문양, 연속된 숫자가 발생했습니다!");
                                        return true; // 1 3 2
                                    }
                                }
                            }
                            if((checkNum - 1 == arr[1][l]) && (checkNum - 1 != -1)) { //내려가면서 순차적으로 작아지는 숫자가 있는지 검사.
                                for(int m=0; m<4; m++) {
                                    if((checkNum - 2 == arr[2][m]) && (checkNum - 2 != -1) || (checkNum + 1 == arr[2][m])) {
                                        System.out.println("다른 문양, 연속된 숫자가 발생했습니다!");
                                        return true; // 3 2 1 || 2 1 3
                                    }
                                }
                            }
                            if((checkNum - 1 == arr[2][l]) && (checkNum - 1 != -1)) {
                                for(int m=0; m<4; m++) {
                                    if((checkNum - 2 == arr[1][m]) && (checkNum - 2 != -1)) {
                                        System.out.println("다른 문양, 연속된 숫자가 발생했습니다!");
                                        return true; // 3 1 2
                                    }
                                }
...
//        for(int arr1 : savedRow) {
//            System.out.print(arr1 + "\t");
//        }
//        System.out.println();
//        for(int arr1 : savedCol) {
//            System.out.print(arr1 + "\t");
//        }
