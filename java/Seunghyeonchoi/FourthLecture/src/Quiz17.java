...
public class Quiz17 {
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 1 ; i != 2; ) {
            ...
            int mydice1 = (int) (Math.floor((Math.random()*6 + 1)));
            int mydice2 = (int) (Math.floor((Math.random()*6 + 1)));
            System.out.println("당신의 첫 번째 주사위 결과는 "+mydice1);
            System.out.println("당신의 두 번째 주사위 결과는 "+mydice1);

            int msum = mydice1 + mydice2;
            System.out.println("총 합은" + msum);
            Thread.sleep(300);

            int pcdice1 = (int) (Math.random()*6 + 1);
            int pcdice2 = (int) (Math.random()*6 + 1);
            System.out.println("컴퓨터의 첫 번째 주사위 결과는 "+pcdice2);
            System.out.println("컴퓨터의 두 번째 주사위 결과는 "+pcdice2);
            int psum = pcdice1 + pcdice2;
