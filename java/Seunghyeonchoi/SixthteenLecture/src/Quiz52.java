...
/*
        System.out.println(""+ Arrays.toString(marketSellList));
        System.out.println(""+ Arrays.toString(marketSellListPrice));
        System.out.println("");
...
*/
        ...
        for (int i = 0; i < userBuyListStock.size(); i++)   {
            sum += (marketSellListPrice[i] * userBuyListStock.get(i));
            int sum2 = (marketSellListPrice[i] * userBuyListStock.get(i));

            System.out.printf("%s %d개 : %d원\n", marketSellList[i], userBuyListStock.get(i), sum2);

        }
        System.out.printf("다 합쳐서 %d원입니다.\n", sum);

        if (myMoney > sum)  {
            System.out.printf("당신의 지갑은 %d원 남았습니다.\n",myMoney - sum);
        }else {
            System.out.println("물건을 살 돈이 없는데요?\n");
