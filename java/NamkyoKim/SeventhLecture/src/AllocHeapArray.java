...
public class AllocHeapArray {
    public static void main(String[] args) {
        ...
        int studenNum = sc.nextInt();

        // 동적할당 되는 데이터를 관리하는 메모리  - Heap
        // new 로 만든 데이터는 전부 Heap에서 관리하게된다.
        // 기존에 우리가 stack에 배열을 만들 경우엔 항상 제약 사항이 발생했다.
        // 어떤 제약 사항? 개수가 고정됨(할당된값으로)

        // Heap 방식의 할당에서는 공간을 필요한만큼 할당할 수 있어서 유연한 접근이 가능하다.
        // 근데 여기에도 제약사항이 있음! stack 보다 느리다.
        ...
        for (int i = 0; i < studenNum; i++){
            // studentNum 만큼 생성되니 해당 학생숫자에 맞게 80 ~ 100점의 점수를 가지도록만듬
            studenArr[i] = (int)(Math.random() * 21 )+ 80;
