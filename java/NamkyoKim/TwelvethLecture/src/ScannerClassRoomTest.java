...
// 6. 또 몇명을 입력할 것인지에 대한 정보가 추가로 필요
...
class Employee {
    ...
    public Employee(String name, int year){
        ...
        pay[0] = (int)(Math.random() * 1101 + 2400); // 초봉 설정 하기위해 0번인덱스가 초봉이기때문에 이런식으로 코드작성
    }
    ...
    // calc_종류_변수 : 변수의 속성에 해당하는 전체값을 전부처리함
    // calcYearPay : pay 변수의 속성 year에 해당하는 전체값을 모두 처리
    // 이 규약은 만들기 나름이고 팀 프로젝트를 하는 경우엔 반드시 필요
    // (왜냐면 내가 만든 매서드를 다른 사람이 사용할 수도 있기 때문)
...
public class ScannerClassRoomTest {
    ...
    // |     Heap     |
    //-----------------
    // |     Data     |  <-------- 이 영역에 잡히는 것은 전역변수로서 역할을 수행함
    //------------------
    // |   Text(Code) |
    //------------------
