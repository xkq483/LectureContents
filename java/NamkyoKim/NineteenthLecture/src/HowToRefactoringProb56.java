...
// 2. 클래스의 엔티티( 업무규칙 - 아키텍처 용어상)를 변경할 경우
//      이 케이스의 경우 그나마 git이 있어서 롤백을 통해 살릴 수 있게 된 것이 근래들어서부터임
class SpecialCardGame{
    final int PATTER_MAX = 3;
    ...
    public SpecialCardGame(){
        ...
        preparedMap = new HashMap[PATTER_MAX];


        dupNumArrCheck = new int[PATTER_MAX][CARD_MAX];
        for (int i = 0; i < PATTER_MAX; i++) {
