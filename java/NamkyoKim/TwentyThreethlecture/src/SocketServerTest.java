...
public class SocketServerTest {
    ...
    public static void main(String[] args) {
        // 포트의 역할 : 서비스 번호
        // 결국 우리가 어떤 서비스에 접근하기 위해서는 무엇을 알아야 하나? 아이피와 포트
        int port = Integer.parseInt("33333");

        try{
            // 소켓이란?
            // 전기 분야에서 소켓에 전원 코드를 연결하면 전기 제품들이 구동 가능한것과 마찬가지로
            // 프로그래밍 분야애서 소켓이란 다른 컴퓨터와 내 컴퓨터를 연결하는 통로 역할을 한다.
            ...
            while (true){
                ...
                // 사용자 준비될때까지 난 계속 기다린다. (문 두드리면서)

                // Blocking의 반대 개념도 있지 않을까?
                // Non_Blocking 이라고 하며 비동기 처리와 관계가 깊음
                // (여기에 있는 sock는 접속한 사용자 소켓)
                Socket sock = servSock.accept();
                // 접속이 완료되었으면 접속한 클라이언트의 IP를 확인함
                ...
                // InputStream을 사용해서 들어오는 객체는 반드시 아래와 같이 읽어야함
