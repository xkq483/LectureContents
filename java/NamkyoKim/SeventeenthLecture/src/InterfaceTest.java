// 인터페이스 작성법
// 1. 일단 interface를 적는다.
// 2. 인터페이스명(일종의 클래스 같은 것이라고 보면됨)을 적는다.
// 3. 인터페이스 내부에는 매서드 프로토타입을 작성한다.
//      ( 프로토 타입이 뭘까? 매서드의 접근제한자 , 리턴타입 , 매서드이름 , 입력등을 기록한 형태)
interface Remocon{
    public void turnOn();
    public void turnOff();
...
// OOP(객체지향)에서 제일 중요시 여기는 것이 바로 추상화이다.
// 현재까지의 내용을 토대로 추상화란 궁긍적으로 무엇을 추구하는것인가 ?

// 복잡하고 어렵고 토나오는것은 우리가 해줄게
// (자바 라이브러리 개발자 진영 및 스프링 프레임워크 개발진영)
...
class AbstractTest{
    Remocon rc = new Remocon() {
        ...
        @Override
        public void turnOff() {
            System.out.println("이제 끌께~ ㅃㅃㅃㅃ");
        }
    };
    Remocon radio = new Remocon() {
        @Override
        public void turnOn() {
            System.out.println("라디오 킬게 주파수 채널 조정해~");
        }

        @Override
        public void turnOff() {
            System.out.println("라디오 전원 차단할게 낼 봥~");
        }
    };
    public void testMethod(){
        Remocon tv = new Remocon() {
            ...
            @Override
            public void turnOff() {
                System.out.println("TV OFFFFFFFFFF~");
            }
        };
        tv.turnOn();
        radio.turnOff();
