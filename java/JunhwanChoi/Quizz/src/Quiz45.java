...
class DiceDeathGame {
    final int MAX_PLAYER = 2; // 플레이어 수 상수로 지정
    final int DICE_IDX = MAX_PLAYER + 1;
    final int START_MONEY = 1000;   // 게임시작머니를 상수로 지정

    final int FIRST_IDX = 0;   // 첫번째 인덱스 0
    final int SECOND_IDX = 1;  // 두번째 인덱스 1
    final int TOTAL_IDX = 2;   // 토탈 인덱스 2
    ...
    public DiceDeathGame () { // 생성자 : 초기화 부분
        roundNum = 0;
    ...
    public void gameStart () { // do while문으로 실행 메소드를 만듬 -> 메인에 때려박지않고 클래스 내에서 메소드로 구현
        do {
            bettingMoney();
            runDice(); // 랜덤 다이스값 + 2번째 주사위 생성조건
            checkMagicDiceSkill();
            whoGetBetMoney();
            canWePlay();
            checkEachMoney();
