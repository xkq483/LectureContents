class RandomWorker{
    ...
    public RandomWorker () {
        yearMoney = new int[MAX];

        for (int i = 0; i < MAX; i++) {
            yearMoney[i] = (int)(Math.random() * 1101 + 2400);
        }
    }
    public void calcRandomSalary (int year) {
        ...
        for (int i = 0; i < year; i++) {
            ...
            yearMoney[i] = (int)(yearMoney[i] * (1.0f + randIncRate * PERCENT));
            // X = A + A * B * C
            // X = A + ABC ===> A * (1 + B * C)
    ...
    public void printSalary () {
        for (int i = 0; i < MAX; i++) {
            System.out.println("연봉을 출력합니다: " + yearMoney[i]);
        }
    }
}
// 랜덤 연봉 적용을 해보자
...
public class Quiz43 {
    public static void main(String[] args) {
        RandomWorker rs = new RandomWorker();
