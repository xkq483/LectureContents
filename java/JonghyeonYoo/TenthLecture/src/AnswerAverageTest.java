class AverageTest {
    ...
    public AverageTest (int scoreArr[]) {  // scoreArr[] 배열을 입력 받는다.
        len = scoreArr.length;

        scores = new int[len];

        for (int i = 0; i < len; i++) {
            scores[i] = scoreArr[i];       // 위 입력받은 scoreArr[]를  scores[]에 할당한다!
        }
    }
    ...
    public void calcMean () {       //평균을 구혀라고 만든 메서드
        float sum = 0;
...
public class AnswerAverageTest {
