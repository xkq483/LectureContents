...
class Market {
    private ArrayList<String> userBuyList;    // 구매 물품
    private ArrayList<Integer> userBuyListStock;  // 구매 수량

    private String[] marketSellList = {"선풍기", "키보드", "마우스", "모니터"};
    private int[] marketSellListPrice = {380000, 80000, 70000, 400000};

    private int myMoney;   // 처음 갖고있는 돈
    private int itemSumPrice; // 물건들의 총 금액
    private int myWallet;   //내 지갑속의 돈
    int shortMoney;         //부족한 금액
    ...
    private void doPayment () {
        ...
        for (int i = 0; i < userBuyList.size(); i++) {
            if (userBuyList.get(i).equals("선풍기")) {
                itemSumPrice = userBuyListStock.get(i) * marketSellListPrice[0];
            } else if (userBuyList.get(i).equals("키보드")) {
                itemSumPrice = userBuyListStock.get(i) * marketSellListPrice[1];
            } else if (userBuyList.get(i).equals("마우스")) {
                itemSumPrice = userBuyListStock.get(i) * marketSellListPrice[2];
            } else if (userBuyList.get(i).equals("모니터")) {
                itemSumPrice = userBuyListStock.get(i) * marketSellListPrice[3];
            }
        }
      ...
      private void wallet() {
          if(myMoney < itemSumPrice) {
            shortMoney = itemSumPrice - myMoney;
            System.out.printf("갖고있는 돈이 부족합니다. %d원 부족\n", shortMoney);
        }else if(myMoney > itemSumPrice) {
              myWallet = myMoney - itemSumPrice;
              System.out.printf("결제가 완료되었습니다. 현재 남은돈은 %d 원 입니다.\n", myWallet);
          }
    }
