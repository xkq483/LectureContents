...
class Employee {
    ...
    public Employee (String name, int year) { //int year로 내가 main함수에 final int year = 값을 설정해줄수있다.
        this.name = name;
        curYear = 0;  //현재 년차는 0으로 초기화

        pay = new int[year];
        incRate = new float[year];

        pay[0] = (int)(Math.random() * 1101 + 2400); //pay[0]번 인덱스에 첫 연봉값이 들어간다.
    }
    ...
    // 위와같은 규약에 따라 그냥 지금 생각나는대로 작성한 부분
    // calc_종류_변수 : 변수의 속성에 해당하는 종류 전체값을 전부 처리함
    // calcYearPay : pay 변수의 속성 year에 해당하는 전체값을 모두 처리
    // 이 규약은 만들기 나름이고 팀 프로젝트를 하는 경우엔 반드시 필요함
    // (왜냐하면 내가 만든 메소드를 다른 사람이 사용할 수도 있기 때문)
    public void calcYearPay () { //년차별 인상률에 따른 연봉 계산 (입력되는 모든 년차를 구하는 메소드)
        for (int i = curYear + 1; i < pay.length; i++) {
               //curYear+1은 현재curYear = 0 값이 들어있기때문에 결과적으로 1년차 부터 시작
            pay[i] = pay[i - 1] + (int)(pay[i - 1] * getRandomIncRate() * PERCENT);
            //pay[i-1]의 이유는 pay[0]인덱스에 초기 연봉값이 들어있기때문
            //pay[1-1] + (int)(pay[i - 1] * getRandomIncRate() * PERCENT);
            //    [0]인덱스 연봉값 + ([0]인덱스 연봉값에 인상률을 곱한값) = pay[1] (2년차 연봉)
            // 2년차 연봉을 계산할 수 있다.
        }
    }
    public void calcPay () { //인상된 연봉 구하기 (1년치만 구하는 메소드)
        //현재 안쓰고있음
        pay[curYear + 1] = pay[curYear] + (int)(pay[curYear] * getRandomIncRate() * PERCENT);
        curYear++;
        //pay[curYear + 1] = 작년연봉 + (작년연봉 *인상률)
        // 2 ~ n년차 연봉
    }
    public void printPay () { // 이름과 년차 연봉까지 출력하는 메소드
        for (int i = 0; i < pay.length; i++) {
            System.out.printf("%s님의 %d년차 연봉은 %d입니다.\n", name, i + 1, pay[i]);
                                // i + 1을 한 이유는 i=0 부터 대입으로 0+1 1년차를 출력하기 위함
        }
    }
