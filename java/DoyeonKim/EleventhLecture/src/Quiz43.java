...
class RandomSalary {
    ...
    public RandomSalary() {
        yearSalary = new int[MAX]; //연봉 10명

        for (int i = 0; i < MAX; i++) {
            yearSalary[i] = (int)(Math.random() * 1101 + 2400); //10명 2400 ~ 3500 으로 연봉 랜덤 지정
        }
    }

    public void  calcRandomSalary (int year) { //10년동안  ↓아래 rs.calcRandomSalary(10); 정해줌
        float randIncRate;  //랜덤 증가

        for (int i = 0; i < year; i++) {
            // 1 ~ 20%
            randIncRate = (float)(Math.random() * 20 + 1);  //연봉 인상률은 1% ~ 20% 사이
            // yearSalary[i] = (int)(yearSalary[i] + yearSalary[i] * randIncRate * PERCENT);
            yearSalary[i] = (int)(yearSalary[i] * (1.0f + randIncRate * PERCENT));
            ////연봉은 = 현재연봉 x 연봉 인상률 1% ~ 20% 사이수 x 0.01
            // X = A + A * B * C
            //X = A + ABC ===> A * (1 + B * C)
    ...
    public void printSalary () {
        for (int i = 0; i < MAX; i++)  {
            System.out.println("연봉을 출력합니다" + yearSalary[i]); //총 10명의 각 10년후 최종 연봉만 출력
        }
    }
}
