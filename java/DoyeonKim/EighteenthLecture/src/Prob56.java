...
public class Prob56 {
    ...
    public static void main(String[] args) {
        ...
        String[] pattern = {"spear", "sword", "arrow"}; //문양

        for (int i = 0; i < 3; i++) {
            preparedMap[i] = new HashMap<String, Integer>();//패턴이랑 인덱스?


            for (int j = 0; j < 10; j++) {
                preparedMap[i].put(pattern[i] + j, j); //10의 카드 생성 문양하나 + 1~10, 1~10 뒤에 j는 왜있는거지
            }
        }
        ...
        System.out.println(map);//전체 문양 카드

        System.out.println("카드를 분배합니다.");

        //같은 패턴에서 같은 숫자가 나오면 안됨
        //그렇기 때문에 중복된 숫자가 존재하는지 엽를 체크하는 배열이 필요하다
        ...
        for (int i = 0; i < 4; i++) {
            ...
            do {
                ...
                // 57번 문제를 풀어보도록 합시다!
                sculptureIdx = (int) (Math.random() * 3); //랜덤 문양 3개
                randNum = (int) (Math.random() * 10); //카드10개
