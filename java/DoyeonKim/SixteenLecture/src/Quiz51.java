...
class Market {
    ...
    public Market () {
        userBuyList = new ArrayList<String>(); //구매목록이름
        userBuyListStock = new ArrayList<Integer>(); //물건을 산 재고
    ...
    private void checkContinueShopping () { //쇼핑을 계속 할것인지
        Boolean isOK = false;
    ...
    private void selectBuyItemStock (String selectItem) {
        ...
        createNonDuplicateBuyList(selectItem, amount); //중복구매목록 이부분 다시 보기 구매수량
    }
    ...
    private void selectBuyItem () {
        ...
        do {
            ...
            selectBuyItemStock(marketSellList[itemNum - DEFAULT_IDX]); //이부분이 어떻게 작동하는지 잘 모르겠습니다

            System.out.println(userBuyList); //구매 물품
            System.out.println(userBuyListStock); //구매 물량
    ...
    private void createNonDuplicateBuyList (String target, int amount) {
        ...
        if (idx == -1) {    // 중복 없음
            userBuyList.add(target);//[키보드, 선풍기, 마우스, 모니터]
            userBuyListStock.add(amount); //[12, 5, 6, 3]

        } else {            // idx가 중복된 요소를 알려줌
            // set(idx, 데이터)는 특정 인덱스의 값을 update(갱신)함
            // add(idx, 데이터) + remove(idx + 1)과 동일한 역할을 함
            userBuyListStock.set(idx, userBuyListStock.get(idx) + amount);//갱신된것?
