...
class ThreadManager {
    ...
    final static int EVEN = 2; // 식별자 짝수들의 합
    final static int SEVEN = 7; //7의 배수들의 합
    final static int ELEVEN = 11;//1의 배수를 곱

    final static int[] OPTION_ARR = { EVEN, SEVEN, ELEVEN }; //각각 수행할 수 있도록 만들어줌
...
class DistributedThread implements Runnable {
    ...
    public void run() {
        ...
        for (BigInteger i = start; i.compareTo(end) == -1; i = i.add(ONE)) {
            ...
            // if (i % option == 0) {
            if (
                    (i.mod(new BigInteger(String.valueOf(option))).//integer로 형변환 해줌
                            compareTo(BigInteger.ZERO)
                    ) == 0
            ) {
                //i와 BigInteger에 있는 옵션값을 나머지연산 취한 결과가 0과 같은지 확인
            // ) {
                /* DEBUG 메시지
                System.out.println("threadIdx: " + threadIdx +
                        ", option: " + option +
