...
class ArrayListCount {
    ...
    ArrayList<Integer> duplicateLists; //중복 카운트
    ...
    public ArrayListCount() {
        ...
        for (int i = 0; i < DATA_LENGTH; i++) {
            intLists.add((int)(Math.random() * RAND_RANGE + RAND_START_OFFSET));//3+10, 10~12
            //intLists.add(10);
        }
    ...
    public void cntDuplicate() {
        ...
        for (int num : intLists) {
            ...
            if (num == FIRST_VALUE) {
                ...
                // 이후 [1, 0, 0, 0]
                // 그 다음은 [2, 1, 0, 0]
                duplicateLists.remove(1); //10이 중복된 횟수
                // 이후 [1, 0, 0]
                // 그 다음은 [2, 0, 0]  암튼 이래서 마지막에 중복된 횟수가 나옴
                // 빼온 값이 11이면
