...
//isTrue가 false라서 while문을 빠져나온다? 그리고 아래를 실행한다.
        System.out.print("몇 명의 학생 점수를 입력하시겠습니까 ? ");
        studentsNum = scan.nextInt(); //입력하면 studentsNum에 대입

        scores = new int[studentsNum];

        for (int i = 0; i < studentsNum; i++) {//입력한 숫자만큼 돌리는 조건
    ...
    public void calcAverage () {
        ...
        mean = (float)sum / (float)scores.length;// 총합 나누기
    }
    public void calcVariance () {
        int sum = 0;

        for (int i = 0; i < scores.length; i++) {
            sum += Math.pow((scores[i] - mean), 2); //총합 빼기 평균..??
        }

        variance = (float)sum / (float)scores.length;
    }
    public void calcStdDeviation () {
        stdDeviation = (float)Math.sqrt(variance); //모르겠다....
    }
