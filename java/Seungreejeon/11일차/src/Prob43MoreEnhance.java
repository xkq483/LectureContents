class SalaryTest2 {
    ...
    public void calcRandomSalary (int year) {
        ...
        for (int i = 0; i < year; i++) { //특정년도까지의 금액
    ...
    public void calcRandomSalary () { //이건 참조형 안받음 즉 1년치
...
public class Prob43MoreEnhance {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < nameLen; i++) {
            st[i] = new SalaryTest2(nameArr[i]); //st[i] 부터 이름이들어간다 생성자 위치의 name을통해 클래스변수에 저장
        }
        ...
        for (int i = 0; i < 10; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                ...
                st[j].calcRandomSalary(); //참조형을 찾아서 1년치만 계산 잘보면 year 참조형
                sum += st[j].getSalary();
