...
class Employee {
    ...
    public void calcYearPay () {
        for (int i = curYear + 1; i < pay.length; i++) {  //0년차 연봉이 아닌 1년차 연봉이라고 표기하기위해 +1
            pay[i] = pay[i - 1] + (int)(pay[i - 1] * getRandomIncRate() * PERCENT); // 다음해연봉 이번해 연봉표기를 위해 -1
        }
    }
