...
//프라이빗과 퍼블릭, 배열을 사용해서 피보나치수열 풀어보기
class Fibonacci {
    private int[] fibArr;
    ...
    public Boolean calcLastElem () {
        if (lastElement <= 0) {
            ...
            for (int i = 2; i < lastElement; i++) {  //int i가 2인 이유 : 첫번째 수와 두번째 수를 제외하고 시작해야 하기 떄문에
                fibArr[i] = fibArr[i - 2] + fibArr[i - 1];  //피보나치함수의 각각의 항에 맞춰진다
            }
        }
