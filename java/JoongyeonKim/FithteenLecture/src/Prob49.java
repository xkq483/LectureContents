...
class ArrayListCount {
    ...
    final int RAND_RANGE = 3;  //  랜덤값 설정할 때 랜덤값의 범위를 3으로 맞추기위해 만든 변수
    final int RAND_START_OFFSET = 10;   // 베열의 인덱스값을 맞추기위해 -10을 하기위한 변수
    final int DATA_LENGTH = 10;   // 랜덤 숫자를 10개 만들기위해 만든 변수

    final int FIRST_VALUE = 10;
    final int SECOND_VALUE = 11;   //10~12 값을 설정
    final int THIRD_VALUE = 12;

    final int FIRST_IDX = FIRST_VALUE - RAND_START_OFFSET;
    final int SECOND_IDX = SECOND_VALUE - RAND_START_OFFSET;  //베열의 인덱스값에 맞추기 위해 모두 -10을 해준다
        ...
        for (int num : intLists) {
            ...
            if (num == FIRST_VALUE) {
                ...
                duplicateLists.add(FIRST_IDX,
                        duplicateLists.get(FIRST_IDX) + 1);  //+1을 하는 이유 :  10이 나오면 중복값에 카운트하기 위해서 (+10을 하면 한번 나올때마다 중복값이 10씩 카운트된다)
                // 이후 [1, 0, 0, 0]
                // 그 다음은 [2, 1, 0, 0]
