...
public class ArrayListTest {
    public static void main(String[] args) {
        ...
        ArrayList<String> fruitsList = new ArrayList<>(Arrays.asList(fruits)); //ArrayList배열 만드는 방법
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // 여기서 index는 | 데이터1 | ---> | 데이터2 | ---> | 데이터8 | --->  //질문 데이터4가 아니라 8인건가요??
            //                   0               1               2
            System.out.printf("get(%d) = %s\n", i, clone.get(i));
        }
        ...
        for (int i = 0; i < 4; i++) {
            System.out.printf("get(%d) = %s\n", i, clone.get(i));  //clone에서는 지워지지않았다 (fruitList를 복제한 것이기에)
        }

        System.out.println("fruitsList는 지워졌을까 ?");

        for (int i = 0; i < 3; i++) {
            System.out.printf("get(%d) = %s\n", i, fruitsList.get(i));  //fruitList에서는 지워졌다
        }
        ...
        System.out.println("사과는 ? " + clone.indexOf("apple"));
