var app = new Vue ({ 
    ...
    buttonClickTest: function(event) {
        ...
        addMonster: function() {
            // 샘플용 코드
            var arr =[1,2,3,4];
            // reduce ( function (유지되는값, 반복하며 비교하거나 연산 대상이 되는 값))
            ...
            // reduce() 사용하면 알아서 내부에 있는 요소를 
            // 전부 낱개로 쪼개서 하나하나 비교하게됨
            // 그래서 function(a, b)에 입력으로 들어가는 a,b는
            // 모두 monsters 배열에 잇는 각각의 낱개 요소들임
            // 계속 낱개 단위로 끝까지 비교를 한다면 
            // 마지막엔 제일 큰 숫자(결국 맨 마지막 녀석)을 얻게 될 것이다.

            // reduce는 전체 순회를 끝낼때까지 동작이 값이 증발되지 않으며
            // 최종 결과를 얻을때까지 이전 결과를 유지하며 연산이 진행됨.
