...
// Controller 는 HTML을 파싱하는 반면
// RestController 는 json을 파싱한다
@RestController
public class SixthController {
    ...
    public String getJsonKeyValueTest () {
        ...
        // 향후 Vue에서 보내는 데이터도 전부 이 json 형태로 날라옴
        // 우리가 SRT API나 여러가지 Rest API라고 하는 녀석들도
        // 요청을 하면 데이터 형식이 전부 json 형식으로 날라옴
    ...
    // josn 내에 Object 형식이 구성된 경우의 피싱
        JSONObject jObj = new JSONObject(jsonString);

        JSONObject movie1Obj = jObj.getJSONObject("movie1");
        log.info("movie: " + movie1Obj.toString() +
                "\ntitle: " + movie1Obj.getString("title") +
                ", draft: " + movie1Obj.getBoolean("draft") +
                ", star : " + movie1Obj.getInt("star"));

        JSONObject movie2Obj = jObj.getJSONObject("movie1");
        log.info("movie: " + movie2Obj.toString() +
