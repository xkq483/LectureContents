var app = new Vue ({
    ...
    data: {
        ...
        list: ['사과', '바나나', '딸기', '수박', '포도', '딸기', '참외', '블루베리', '체리'],
        show: true,
        num: 3,
        count: 0,
        radius: 50,
        randomNumber: 0,
        monsterName: '',
        monsterList: [{'오크', '엘프', '히드라', '고질라', '인간', '난쟁이', '킹콩', '백터맨', '스파이더맨'}],
        monsters: [
            { id: 1, name: '슬라임', hp: 30 },
            { id: 2, name: '고블린', hp: 50 },
    ...
    methods: {
        ...
        addMonster: function () {
            // 이해를 돕기 위한 샘플용 코드
            var arr = [1, 2, 3, 4];
            // reduce ( function (유지되는 값, 반복 비교하거나 연산 대상이 되는 값))
            ...
            // 그리고 주의해야 할점은 reduce는 전체 순회를 끝낼때까지 동작이 값이 증발되지 않으며
    ...
    function randomMonster () {
            this.monsterList = Math.floor(math.random() * 10) + 1;
        }
    },
    beforeCreate() {
