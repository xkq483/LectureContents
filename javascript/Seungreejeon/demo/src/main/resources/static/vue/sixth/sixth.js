var app = new Vue ({
    ...
    methods: {
        ...
        jobskill () {
            for (var i = 0; i < this.monsters.length; i++) {
                this.monsters[i].hp = this.monsters[i].hp - this.characterStatus.atk * 444
            }
        }
    },
    beforeCreate() {
    ...
    beforeUpdate() {
        ...
        if ((this.characterStatus.level >= 30) && (this.characterStatus.selectJob === '모험가')) {
        ...
        for (; this.characterStatus.currentLevelBar >= this.characterStatus.totalLevelBar; ) {
            ...
            if (this.characterStatus.selectJob === '모험가') {
                ...
                this.jobskillshow = true
                if ( this.jobskill === true){
                    alert('싸대기 스킬이 활성화 되었습니다')
                }
            }
